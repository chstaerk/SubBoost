...
External_LOOCV = function (i,data,...){
  ...
  if (marginal.screening) {
    marg.corr = numeric(p)
    for (j in 1:p) marg.corr[j] = abs(cor(data.cur$x[,j],data.cur$y))

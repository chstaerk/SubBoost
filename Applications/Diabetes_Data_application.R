...
External_LOOCV = function (i,data,...){
  ...
  # AdaSubBoost 
  start.time <- Sys.time()
  output = AdaSubBoost(data=data.cur, Iter=Iter, size.fixed=size.fixed, K=K, q=q, tau = tau, const=const, U_C=20, savings=1, nstop=nstop, s_max=min(p,20))
  end.time <- Sys.time()
  ...
  outputRSub = AdaSubBoost(data=data.cur, Iter=Iter, size.fixed=size.fixed, K=K, q=q, tau = tau, const=const, U_C=20, savings=1,
                           adaptive=FALSE, nstop=nstop, s_max=min(p,20))
  end.time <- Sys.time()
...
set.seed(1)
LOOCV_results = mclapply(1:2, External_LOOCV, mc.cores = numCores, data=data, mstop.max = mstop.max,Iter=Iter,K=K,q=q, tau = tau, const=const, U_C=20, savings=1, mc.set.seed = TRUE,  mc.preschedule = FALSE, size.fixed=size.fixed, q_stabsel=q_stabsel, PFER=PFER, nstop=nstop)

save(LOOCV_results, file="LOOCV_Diabetes_CV.RData")
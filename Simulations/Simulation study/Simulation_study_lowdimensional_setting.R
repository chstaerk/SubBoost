...
fsim <- function(i, Iter, K, q, size.fixed, tau, const, U_C=25, savings=1, PFER, q_stabsel,
  ...
  # AdaSubBoost 
  start.time <- Sys.time()
  output = AdaSubBoost(data=data, Iter=Iter, size.fixed=size.fixed, K=K, q=q, s_max = s_max, tau = tau, const=const, U_C=U_C, savings=savings, nstop=nstop, plotting=FALSE)
  end.time <- Sys.time()
  ...
  start.time <- Sys.time()
  outputRSub = AdaSubBoost(data=data, Iter=Iter, size.fixed=size.fixed, K=K, q=q, s_max = s_max, tau = tau, const=const, U_C=U_C, savings=savings,

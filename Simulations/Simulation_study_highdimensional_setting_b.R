...
fsim <- function(i, Iter, K, q, size.fixed, tau, const, U_C=20, savings=1, PFER, q_stabsel,
                 s0, n, p, n_test, sigma.normal, corr, nstop, randomS0){
  set.seed(i)
  if (randomS0) {
  ...
  # AdaSubBoost 
  start.time <- Sys.time()
  output = AdaSubBoost(data=data, Iter=Iter, size.fixed=size.fixed, K=K, q=q, tau = tau, const=const, U_C=20, savings=savings, nstop=nstop, plotting=FALSE)
  end.time <- Sys.time()
  ...
  outputRSub = AdaSubBoost(data=data, Iter=Iter, size.fixed=size.fixed, K=K, q=q, tau = tau, const=const, U_C=20, savings=savings,
                           adaptive=FALSE, nstop=nstop, plotting=FALSE)
  end.time <- Sys.time()
  ...
  #results = list(AdaSubBoost_results = AdaSubBoost_results, 
  #               RSubBoost_results = RSubBoost_results,
  #               SubBoost_results = SubBoost_results,
...
set.seed(1)
results = mclapply(1:nSim, fsim, Iter=Iter, mc.cores = numCores, K = K, q = q, size.fixed = size.fixed, tau = tau, const = const, nstop = nstop, U_C=20, savings=1, PFER = PFER, q_stabsel = q_stabsel, 
                   s0 = s0, n = n, p = p, n_test = n_test, sigma.normal = sigma.normal, corr = corr, mc.set.seed = TRUE,  mc.preschedule = FALSE , randomS0=randomS0)
...
save(results, file="Sim_Toeplitz08_p1000_n100_nSim500_Iter5000_randomS0_10_autostop.RData")
